  ...
  return class extends React.Component {
    ...
    makeSocket() {
      let socket = new WebSocket(
        "wss://connect.websocket.in/spec?room_id=queue-" +
          (window.location.pathname.slice(1) || 1)
      );
      socket.addEventListener("message", message => {
        let parsed = JSON.parse(message.data);
        if (
          parsed.intent === "look for presenter" &&
    ...
    async appendToChat(chat) {
      if (chat.message === "/table" && this.state.role === "presenter") {
        this.setState(state => ({
          table: !state.table
        }));
      } else if (chat.message === "/rick" && this.state.role === "presenter") {
    ...
    onRemove(index) {
      let messages = remove(this.state.messages, index);
    ...
    onDragEnd(result) {
      ...
      if (!result.destination) {
        messages = remove(this.state.messages, result.source.index);
      } else
        messages = reorder(
          this.state.messages,
          result.source.index,
